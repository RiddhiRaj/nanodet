#this file only to be run on RK35xx hardware for yolov8 inference
...
def letterbox(im, new_shape):
    ...
    Args:
        ...
        new_shape: Target shape as (height, width) tuple or int for square
    
    Returns:
        out: Letterboxed image
...
def box_process(position, input_size):
    """
    Process box predictions with support for rectangular inputs.
    
    Args:
        position: Box position tensor
        input_size: (height, width) tuple or int for square
    """
